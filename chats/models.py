...
class MessageManager(models.Manager):
    ...
    def fetch_messages(self, sender, receiver):
        first_lookup = Q(sender__email = sender , receiver__email = receiver)
        second_lookup = Q(sender__email = receiver , receiver__email = sender)
        return self.get_queryset().filter(first_lookup | second_lookup).order_by('time_stamp')

class Message(models.Model):
    sender = models.ForeignKey(CustomUser , on_delete=models.CASCADE, related_name='sendder')
...
class ChatThread(models.Model):
    ...
    def __str__(self):
        return f"latest chat thread of {self.first_user.full_name} and {self.second_user.full_name}"